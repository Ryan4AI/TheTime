...
function buildSystemPrompt(state, monthEvent) {
  ...
  return [
    ...
    ``,
    `# 质量自检铁律（v10 新增 · D007 · 每轮生成完毕输出 JSON 前的最后一道关）`,
    ``,
    `生成完每个分支的 content / options / patch 后、输出 JSON 之前，按以下 9 条逐条自检。任何 1 条不满足，回到 content 重写那一分支。`,
    `自检在内心完成，不要在输出里写"我已自检"。`,
    ``,
    `1. **声音契约（the-storytellers-workbench 借鉴）**：本分支出现的每个 NPC，他们的台词、动作、关注点必须符合他们的人物设定。粗人不讲商业术语，7 岁孩子不懂"分包""外包"等现代词汇，商人关心行情但不议论朝政，女人不议论丈夫。`,
    `2. **戏剧张力（the-storytellers-workbench + story-structure-builder 借鉴）**：3 个选项之间，关键动词不重复 >2 次。p 值差异不要悬殊到 0.7/0.2/0.1（容易暗示玩家该选哪个）；p 值极端允许但剧情分支的"代价/收益"应有真实差异，让玩家犹豫。`,
    `3. **物品一致（v9 铁律 #4 升级）**：content 中提到的物品 ⊆ state.items。如果要写"丢失/损毁"，必须在 patch.items 显式声明（如 {"lighter":"lost"}）。`,
    `4. **阶层一致（v9 写作风格 + story-cog Genre expectations 借鉴）**：庶人进不了皇宫、考场、官署、道观；商人不能穿绸缎；7 岁不能喝酒、不能上赌桌；女人不能进考场。content 中出现的场景必须符合 state.socialClass 边界。`,
    `5. **具体细节（story-cog + the-storytellers-workbench 借鉴）**：本分支至少出现 1 件具体实物（物件名/菜名/地名/动作），禁止 3 个以上连续形容词堆砌（如"古朴典雅的陈设" → "半塌的木桌上一碗凉粥"）。`,
    `6. **戏剧问题（writing-claw "Every scene has a dramatic question" 借鉴）**：本分支能回答 1 个"这一刻玩家要决定什么"的具体问题。写不出来说明本分支没意义。`,
    `7. **场景动 2 件事（writing-claw 借鉴）**：本分支要么推进剧情+深化角色，要么推进角色+切换情绪，要么揭示信息+复杂化。单功能的场景（只讲故事 / 只交代状态）删掉重写。`,
    `8. **信息密度（v9 写作风格 + inkos length governance 借鉴）**：本分支字数控制在 200-400，禁注水。超过 450 必有冗余。低于 150 必有信息缺失。`,
    `9. **爽点密度（novel-writers 借鉴 + design.md 真实定位）**：200-400 字 content 里至少有 1 处"反转/揭晓/发现/反差"（不一定是大事，可以是 NPC 的一句话、环境的细节、物品的发现）。穿越日记是"低密度高烈度"爽点——不是番茄"每段一小爽点"，而是"几十轮积累 → 大爽点"。本规则不压制爽点，要求每轮至少埋 1 个小钩子。`,
    ``,
    `# 不可触碰的硬性禁忌词（v10 新增）`,
    ``,
    ...
    `- 现代物品当古文用："打火机"在唐宋说"火折子"、"急救包"说"药囊"——但 state.items 里的现代物品名在叙事中**保持原名**（如"打火机"），不要用古名替代，只是不要在对话里让古人"懂"打火机的现代含义。`,
    ``,
    `# 当前状态`,
