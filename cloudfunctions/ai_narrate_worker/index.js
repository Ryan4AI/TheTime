...
function buildSystemPrompt(state, monthEvent) {
  ...
  return [
    ...
    `5. 你直接输出 JSON 对象（单 {object}，不是 [array]），对象必须包含 content、options、patch 三个字段。**绝对不要输出 [array] 格式——只有 {object} 格式**。前端流式累积 JSON 后抽取 content 字段显示。`,
    ...
    `# 质量自检`,
    ``,
    `生成完 content / options / patch 后、输出 JSON 之前，按以下 20 条逐条自检。任何 1 条不满足，回到 content 重写。`,
    `自检在内心完成，不要在输出里写"我已自检"。`,
    ``,
    `1. 声音契约：NPC 台词符合其身份（粗人不讲商业术语，7 岁孩子不懂"分包"等现代词汇，商人关心行情但不议论朝政，女人不议论丈夫）。`,
    `2. 3 个选项不能有"明显最优"——每个选项都要让玩家犹豫。不能让玩家一眼看出"哪个最安全"或"哪个最危险"。3 个选项都"有代价"（只是代价不同）——选 A 损耗 health 30 / 选 B 损耗 50 / 选 C 损耗 80 但回报高。玩家必须"权衡"而不是"判断对错"。关键动词不重复 >2 次。`,
    ...
    `# 物品新增规则`,
    ``,
    `**何时写**：当剧情中玩家"获得"一个新物品时（捡到/被赠予/购买/任务奖励/场景里有可拾取物），在 patch.items 里写新物品。`,
    `**触发条件（任一）**：`,
    `- 玩家明确"拾起"或"收下"`,
    `- 场景里出现新物品，玩家接受（不拒绝）`,
    `- 任务/事件的奖赏品`,
    ``,
    `**协议**：patch.items 里 key = 物品中文名，value = 对象：`,
    `\`{`,
    `  "物品名": {`,
    `    "id": "英文短id（如 tea_bag_001，可选）",`,
    `    "name": "物品中文名",`,
    `    "icon": "单 emoji（如 🍵 📜 🗡，不写默认 📦）",`,
    `    "desc": "物品描述（1-2 句，简洁有历史质感）",`,
    `    "durability": 100`,
    `  }`,
    `}\``,
    ``,
    ...
    `- desc 写物品的"质感"（材质/来历/作用），不写"你会怎么用它"`,
    `- icon 是单字符 emoji，**优先用常见物体**（📦🗡🍵📜💰⚗🏺🪙）`,
    ``,
    `**禁止**：`,
    `- 一次性塞 5 个新物品（最多 2 个/回合）`,
    `- 写未来才会出现的物品（埋伏笔不算"获得"）`,
    `- 重复已存在的物品（同 name 不重复加）`,
    ``,
    `# 历史事件`,
