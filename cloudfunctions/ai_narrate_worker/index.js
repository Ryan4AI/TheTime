...
async function backgroundTask(request_id, payload) {
  ...
  try {
    ...
    if (e.parseFailed && e.debugInfo) {
      const fakeResult = {
        success: false,
        error: 'AI输出无法解析为分支数组: ' + e.debugInfo.parse_error,
        branch: null,
        branches: null,
        state: null,
        debug: {
      ...
      await safeWriteResult(request_id, JSON.stringify(fakeResult), 'AI输出无法解析为分支数组: ' + e.debugInfo.parse_error + ' | raw[:1500]=' + e.debugInfo.raw_response.substring(0, 1500))
...
async function callAI(state, input, history, monthEvent, isRetry) {
  ...
  try {
    ...
    const specialError = new Error('AI输出无法解析为分支数组: ' + e.message)
