 ...
 *
 * v0.1.76 — 彻底换思路：
 * 旧方案问题：worker 写 narrate_pending.result 字段 → NoSQL 报错（field 名冲突）+ CAP 滞后
 *
 * 新方案：worker 写一个独立的 narrate_result 集合（每个 request_id 一条独立文档）
 * narrate_result 集合 schema 固定（_id / result_str / error_str / created_at），无动态字段
 * 前端 polling 只读 narrate_result，不读写 narrate_pending
 */
