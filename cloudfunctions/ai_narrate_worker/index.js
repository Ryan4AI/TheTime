 ...
 *
 * D008 异步轮询方案 — 第 2 步：实际推理
 * 读 pending → 调 LLM → 解析 → 写回 pending
 *
 * 设计要点：
 * - timeout 120s（兜底够用）
 * - 复用 ai_narrate 的所有逻辑（prompt 构建、LLM 调用、JSON 解析）
 * - 失败也要写 status='error' + 错误信息（前端轮询能感知）
 *
 * 输入：{ request_id: "narrate_xxx" }
 * 输出：{ success: true, elapsed_ms: ... }
 */
...
// v0.1.71 把 ai_narrate timeout 调到 60s；worker 120s 兜底
...
exports.main = async (event) => {
  ...
  try {
    ...
    if (!record) {
      return { error: 'request_id 不存在', code: 404 }
    }

    // 防止重复触发：status 已经是 done/error 时直接返回
    if (record.status === 'done') {
      return { success: true, status: 'already_done', elapsed_ms: Date.now() - startTs }
    }
    if (record.status === 'error') {
      return { success: false, status: 'already_error', error: record.error }
    }

    const { state, input, history, is_retry } = record.payload

    // 2. 推进月份 / 查事件
    const realInput = is_retry ? '' : (input || '')
    const updated = is_retry ? state : updateState(state, input)
    const monthEvent = await queryMonthEvent(updated)

    // 3. 调 LLM
    const { branches, systemPrompt, userPrompt, messages, rawContent } = await callAI(updated, realInput, history, monthEvent, is_retry)
    ...
    // 6. 写回 pending
    await db.collection('narrate_pending').doc(request_id).update({
      data: {
        status: 'done',
        result: result,
    ...
    try {
      await db.collection('narrate_pending').doc(request_id).update({
        data: {
          status: 'error',
          error: e.message || 'AI服务暂不可用',
          finished_at: Date.now(),
        },
      })
    } catch (writeErr) {
      console.error('[ai_narrate_worker] 写 error 状态失败:', writeErr.message)
    }
...
async function queryMonthEvent(state) {
  try {
    let res = await db.collection('event')
      .where({ year: state.year, month: state.month })
      .get()
...
async function callAI(state, input, history, monthEvent, isRetry) {
  ...
  try {
    ...
    console.error('[ai_narrate_worker] 清洗后 cleaned(完整):', cleaned)
...
// ─────── 构建 System Prompt（与 ai_narrate 完全一致）──────
// v10 质量自检 20 条 + 铁律 9 条 + 写作风格 + 节奏段 + 禁忌词 + 输出格式
// 注释略（与 ai_narrate/index.js 同步），完整规则在 ai_narrate/index.js
function buildSystemPrompt(state, monthEvent) {
  ...
  return [
    ...
    `每世之间是"隔代遗传"的关系：玩家不知道自己为什么会"莫名熟悉"某些东西。`,
    ``,
    `**AI 写每一世剧情时的责任**：`,
    `- 跨世痕迹要在**某些轮次**自然出现（不要每轮都用，也不要永不出现）`,
    `- 痕迹的形式不限于下面这些——AI 可以自由发挥，只要玩家感受到"莫名熟悉"就行：`,
    `  - **场景**：走进某座城觉得某条巷子很熟悉（前世在此生活过）`,
    `  - **文字**：在某本书里看到一段似曾相识的句子（前世写过的诗 / 留下的日记）`,
    `  - **物品**：碰到某件物品觉得"这东西好像在哪见过"（前世丢的打火机 / 留下的指南针）`,
    `  - **人**：碰到自称某派传人的人（前世收过的徒弟）`,
    `  - **对话**：NPC 说了句让你心里一动的话（前世听过类似的话）`,
    `  - **梦境**：梦中反复出现一个场景（前世的关键记忆）`,
    `- 痕迹**绝对不能**让玩家知道"这是上世的我"——AI 也不能在 content 里写"穿越""上世"等元叙事术语`,
    `- 痕迹是"自然巧合"——玩家**可以完全不察觉**——不影响游戏体验`,
    ``,
    `**AI 写作时的硬约束**：`,
    `- 写每世剧情时**偶尔**埋跨世痕迹（不要每轮都用，**3-5 轮回 1 次自然呼应**）`,
    `- 跨世痕迹是"隐性"的——玩家能感受到"巧合"但不知道"为什么"`,
    `- 这一世和下一世**剧情独立**——AI 只管这一世写好，不预设"上世影响了什么"`,
    `- 跨世流程（epitaph/legacy/last_words）的处理**不在本 prompt 范围**——AI 不需要管`,
    ``,
    `# 世界观：穿越的真实`,
    ``,
    `玩家是普通人，穿越到真实的中国历史。`,
    ...
    `19. 危险就在身边：危险不能是"远方的"（听说有瘟疫 / 听说有兵乱）。危险必须已经发生在玩家身边，或者正在逼近玩家。玩家必须做出选择——选错就有代价（health 减 / 物品损耗 / 关系破裂 / 错过机会）。选对也只是活下来——下一波危险还会来。`,
