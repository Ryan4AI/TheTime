...
async function callScoringAI(content, prevState, history, playerInput) {
  ...
  const scorePrompt = [
    ...
    `- 物品名要"汉化、有时代感"（茶包/旧锄头/兵书/伤药/火镰/铜钱串）`,
    ``,
    `## D. city / location / occupation（地点/职业变更，无变化不写）`,
    `- city：玩家【抵达并落脚】一个新的城镇·村寨·集镇·城池时，必须写新地名`,
    `  - 例："你乘船南下，三日后到了杭州" → city:"杭州"`,
    `  - 例："走了月余，到榕溪集镇歇脚" → city:"榕溪集镇"`,
    `- **判定标准（满足任一条就必须写）**：`,
    `  1. 剧情写明抵达并停留——住宿/歇脚/投宿/安顿/住下/入城/进村/扎营`,
    `  2. 本回合结束时玩家身在一个新地方，且短期内不返回出发地`,
    `- **不算变更（不写）**：只是路过、只是提到某个地名、只是打算去还没到`,
    `- 快照里的"所在地"是当前值：剧情落脚地 ≠ 所在地 → 必须写新的 city；相同则不写`,
    `- location：同城内换具体场所（如"搬进城西的院子" → location:"城西院子"）`,
    `- occupation：职业明确变化时写新职业（如"被县令聘为幕僚"/"拜师学医"）`,
    `- 无变化不写这些字段`,
    ``,
    ...
    `8. 检查地点/职业：本回合结束时玩家身在何处？和快照"所在地"还一样吗？落脚在新城镇 → 必须写 city（这是高频漏项：逐字扫剧情里的"到了/歇脚/住下/入城"等落脚动作，扫到就写）；职业是否变化？`,
