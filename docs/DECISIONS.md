...
# 2026-07-22 之后补录（PMO 2026-08-03 落档，D094 之前缺失的 D051-D093 历史决策未补，仅录重大架构决策）

## D095（2026-07-22~28 先生拍板）两阶段架构：AI₁/AI₂ 独立云函数调用（内部代号 D094-async）

### 背景
D089/D090 改造后 AI₁/AI₂ 仍纠缠在同一次调用流程里，属性结算依赖主函数返回后的悬空 Promise，导致 llm_io 长期停在 partial、属性和年月不刷新。

### 定案
- AI₁/AI₂ 拆成**两次独立云函数调用**（phase=ai1 / phase=ai2），前端分两次调 worker，每次调用只跑一次 LLM，不是同一实例串行。
- AI₁ 剧情+选项先入库 narrate_history 再返回 partial；AI₂ 读唯一待结算的已入库 AI 消息做评分结算，写回 player_life 并标记已结算。
- 前端继续 partial→final 展示流程，数据源为业务结果接口，不读 llm_io。

### 状态
- 已实施部署（7/28 定型，后续 8 月全部迭代基于此架构）。

## D096（2026-08-02 03:57 先生拍板「一起改吧」）死亡判定重构：AI₂ 直接判死 + 废除 health（死神追杀机制终结）

### 背景（先生 03:32-03:57 连续追问逼出）
1. **社会性死亡判定是死代码**：allZero 判定只写在 finalizeTask，但 D089 后该函数从未被调用 → 社会性死亡从未生效。
2. **D031 删了 patch.health / patch.coin** → AI 输出没有 health 通道，但 prompt 还写着「通过意外、疾病、战乱导致 health 归零」→ AI 无法造成意外死亡。
3. health 归零只剩寿限 + 跨年自然衰减两条路，本质是玩家不可见的死期倒计时。

### 方案演进
- 方案 A（加回 patch.health，AI 自由数字，风险=乱扣血）→ 方案 C（档位制查表映射，防滥用）→ **方案 D（AI₂ 直接判死）**：AI₂ 读全剧情+前情+属性，最清楚玩家遭遇，比 health 通道/档位映射都简单。先生 03:57 拍板方案 D + 废除 health 一起改。

### 定案
1. **AI₂ prompt 加 E 类 death 输出**：`death: {"type":"意外|寿终|社会性","reason":"..."}`。铁律：只有剧情**明文写出致命伤害/必死境地**才可判（一刀穿心/坠崖/瘟疫气绝）；「身受重伤/命悬一线」=濒死不算；玩家选择逃跑/躲避/求救且剧情成功脱险 → 不判死（尊重玩家选择）。判死时属性照常结算。
2. **死亡判定 3 条（全在 worker runPhase2，寿限覆盖之后）**：
   - ① AI₂ 剧情判死（attrPatch.death 存在 → alive=false + deathReason=reason，优先）
   - ② 寿限：age ≥ lifespan → alive=false + deathReason=寿终正寝 + epitaph 兜底（D009 按身份/年龄补默认墓志铭）
   - ③ 社会性：8 属性全归零（<15 岁豁免）→ alive=false + 默认 epitaph
   - 不再有 health=0 中间步骤。
3. **废除 health 字段**：删跨年衰减块、applyPatch 默认值、saveLife/loadLatestLife 字段、前端 9 处、identity.js 3 处、player_save/generate_identity 校验与初始化；`s.alive = s.health > 0` 改为保持原值，alive 由 runPhase2 显式控制。
4. **死神追杀机制终结**：原「死神追杀机制」（computeDeathThreat/calcCompositeScore 动态威胁值，设计完整但从未部署）**正式废弃**——死亡判定不再需要动态威胁值，后续不再提。
5. prompt 物理死亡段改写：AI₁ 可以写致命结果/濒死紧张，但**不写「你死了」**，最终判定由 AI₂ 按剧情结果决定。

### 状态
- 已实施部署（v3.0.66「废除health+AI2直接判死」，2026-08-02 04:15）。

## D097（2026-08-02 04:15 先生报 bug 后修复）物品协议补「移除」语义

### 问题
剧情里铁器被抢，前端物品栏还一直显示。根因：applyPatch items 协议「数字 = 减耐久」，AI₂ 输出 `{"第五只铁器":0}` 表达「没了」，代码解释成「耐久 +0」→ 物品原样保留。**协议里没有「移除物品」的表达方式**。

### 定案
- applyPatch：`change === 0` → 移除该物品（filter by name/id）；负数 → 减耐久（≤0 自动移除）不变；对象 → 新增（同名去重）。
- prompt C 类 items 段改写：移除/被夺走/丢失/损毁/用完 → 写 `{"物品名": 0}`；损耗耐久 → 写负数（如 -20）。

### 状态
- 已实施部署（2026-08-02 凌晨）。

## D098（2026-08-03 00:53-00:58 先生拍板）摘要输入/输出完全不截断

### 背景
先生连续质疑「100 条对话怎么会爆炸」「膨胀应该用 prompt/max_token 限制，为什么无脑截断」。真相：MiniMax M2.7 窗口 200K token，2 万字符输入远不会炸窗口；当初截断是怕云函数 15s 超时（309 条 5.5 万字符实测 13.97s），但摘要异步跑不阻塞叙事，输入全量可控。

### 定案
1. 摘要输入完全不限制：删每条截 300 / 总长截 20000 / 旧摘要截 600。
2. 摘要结果不无脑截 500：改 >800 字才截，且截到最后一个句号（保句子完整）。最终拍板：完全不截断，prompt 软约束 300-500 字 + max_tokens=1500 兜底。
3. 遗留：喂 AI₁ 的 system 消息截 500 纯防御（实际只有十几个字，基本不触发）。

### 状态
- 已实施部署（2026-08-03 00:56/00:58 两次）。

## D099（2026-08-03 01:32 先生拍板）压缩触发规则重设计

### 定案
- 喂 AI = 摘要 + 进度点后**全部原文**（不再 slice 最近 100 条）→ 信息零丢失。
- 触发 = **每到整百编号 n 压到 n-100**（seq=200→压到100，300→压到200...），保证 ≥100 条完整对话。
- 删「无摘要降级截断」（会丢前 99 条）；COMPRESS_TRIGGER 常量删除。
- narrate_history 加自增 seq（4 处写入点 + 存量补齐 + 旧压缩记录补 last_seq），DBG「压缩」tab 显示「压缩到第 N 条」，进度判断 O(1) 不再全表扫。
- 存量 last_seq=213 自动归位：涨到 400 时压到 300。

### 状态
- 已实施部署（2026-08-03 01:34 worker；seq 迁移函数执行后已删除）。

## D100（2026-08-03 02:30 先生拍板）system 消息补全：年号/年龄/物品变化

### 背景
先生反馈「system 消息没有年号/年龄/物品变化」。之前 emitSystemMessages 只有时间（年月）、地点、身份、9 属性，且外部触发 if 会挡掉地点/身份变化。

### 定案
- 时间行带年号（显德四年）+ 跨年带年龄（年龄24岁）。
- 物品变化段：获得X(耐久N) / X耐久 a→b / 失去X。
- **删外部触发 if**（之前只月份/属性/物品变化才发，地点/身份变化被挡掉丢失）→ 无条件调 emitSystemMessages，内部 lines 为空自动返回 []。语义 = 只有 AI₂ 输出无任何变化才没有 system 消息。

### 状态
- 已实施部署（2026-08-03 02:26-02:30 worker）。

## 附：D051-D093 历史决策未补录说明
DECISIONS.md 在 D048/D050/D062 之后直接跳到 D090/D094，中间 D051-D093 大量决策（含 D089 两阶段化、D090-hotfix 系列、D094-async 调试链等）缺失。历史欠账暂不补录，后续新决策从 D095 起正常落档（本批已补 D095-D100）。如需补历史，另行安排。
