# Prompt v11 — 当前线上版本（2026-06-11 D008 拍板后定案）

> 这是 worker 实际运行的 system prompt，由 `buildSystemPrompt()` 动态生成。
> 与代码 `cloudfunctions/ai_narrate_worker/index.js` 的 buildSystemPrompt() 一一对应。
> 上一个版本 v9 见 `docs/prompt.md`（2026-06-07 之前）。

---

## 版本演进

- **v9（2026-06-07 之前）**：`docs/prompt.md` —— 5条铁律 + 写作风格 + 输出格式
- **v10（2026-06-07）**：v9 + 9 条自检铁律 + 1 段禁忌词（D007 拍板）
- **v11（2026-06-11）**：v10 + month_delta 字段说明 + 节奏指导段（D008 拍板）

## D004 决策锁（铁律 + 写作风格 + 输出格式）

v9 的 5 条铁律 + 写作风格 + 输出格式 **不可修改**，只追加新段。

D008 追加的 month_delta / 节奏指导是 **新增段**，不违反 D004 精神。

---

## 完整 Prompt 模板（v11）

下面是 worker 实际发给 MiniMax-M2.7-highspeed 的 system 消息内容：
...
- 跨世流程：epitaph 由 AI 在寿限到时生成（见 D009 墓志铭规则段），legacy 由系统跨世传递
...
历史消息中可能出现的 [system · XXX] 提示是系统自动注入的状态变化，不是玩家说的话，也不是 NPC 的台词。AI 应该把这类消息当作叙事元数据读取（不参与剧情、不该在 content 里复述），AI 也不该自己新写一条 [system · 提示。
...
- D031（2026-06-27 23:13 拍板）：删 coin / health 字段。铜钱 = "财富" 属性（同一回事不重复）。前端不显示 health, patch 没必要。
- 9 属性（声望/财富/学识/颜值/医术/战功/文采/政绩/义行）由 AI₂ 单独评分，不由 AI₁ 在 patch 里写。
- items：物品状态变化。
...
| 占位符 | 来源 | 示例 |
|------|------|------|
| `{state_summary}` | state 对象字段拼装 | 见下方 |
| `{legacy_context}` | `state.legacy` | "这是你第一次穿越，没有前世痕迹。" |
| `{events_context}` | `monthEvent`（查 event 集合） | "本月发生的历史事件：\n标题：...\n描述：...\n影响范围：..." |

### state_summary 模板

```
- 世数：第${life_number || 1}世
- 姓名：${name}，${gender}，${age}岁（${healthDesc}）
- 职业：${occupation}，阶层：${socialClass}
- 朝代：${dynasty} · ${eraDisplay}
- 位置：${city} · ${monthStr}
- 年份：${year}年（第${life_number || 1}世）
- 金钱：${coin}文
- 携带物品：${itemsList || '无'}
```

`healthDesc` 阈值：≥80 精力充沛 / ≥60 还算硬朗 / ≥40 时有小病 / ≥20 体弱多病 / <20 病入膏肓
`monthStr`：正月~腊月（12 个月中文名）

---

## 关键变更摘要（v9 → v11）

###v10（2026-06-07）追加- 9 条自检铁律（自检段）
- 1 段禁忌词（现代商业 / 穿越元概念 / 网络用语 / 现代物品）

###v11（2026-06-11）追加
- patch 字段：`month_delta`（0~60）
- 节奏指导段（回合内 / 次日 / 季节 / 年 / 极长 共 5 档）

###worker 行为变化（D008）
- worker 之前默认 month+1，现在 AI patch.month_delta 决定
- AI 漏字段保底 0（尊重 AI 决定）
- month_delta clamp [0, 60]
- 状态变化触发 system message（5 类并入 1 条）：时间/地点/身份/气血（≥10）/九属性（任一变化列全部）
- D032（2026-06-27 23:20 先生拍板）：财富/其他 8 属性不设阈值，任意变化都列

---

## 关联文档

- `docs/DECISIONS.md` —— D004（v9 铁律定案）/ D007（v10 追加）/ D008（v11 月推进）
- `docs/prompt.md` —— v9 静态版（已过时，保留作历史）
- `docs/D008-async-polling.md` —— 异步轮询方案技术文档
- `cloudfunctions/ai_narrate_worker/index.js` —— 实际代码（line 404-477）

## 维护规则

- 修改 system prompt **必须开新 D 编号**条目（不能修改已存在的决策）
- 改完 prompt **必须同步更新本文件**，保持与代码一致
- 改 prompt 前 **必须先生拍板**（D004 铁律 + 写作风格 + 输出格式 不动）