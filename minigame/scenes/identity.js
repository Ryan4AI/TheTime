...
function init(items, identity) {
  ...
  if (identity) {
    ...
    // ⚠️ 正则须覆盖所有真实历史职业，无兜底
    // 数据库 236 职业 + 通用池 12 职业全覆盖审计：2026-06-15
    var occProfiles = [
      { rx: /医|药|针灸|郎中/, w: { 声望:0.3, 财富:0.3, 学识:0.6, 颜值:0.3, 医术:2.8, 战功:0, 文采:0.2, 政绩:0.1, 义行:0.5 }},
      { rx: /将|兵|军|武|侠|卒|虎贲|甲士|师旅|御手|车右/, w: { 声望:0.8, 财富:0.2, 学识:0.2, 颜值:0.4, 医术:0.3, 战功:2.8, 文采:0, 政绩:0.5, 义行:0.5 }},
      { rx: /书|诗|文|画|儒|墨|秀才|游士|游说|策士|谋士|幕僚|族士/, w: { 声望:0.5, 财富:0.1, 学识:1.2, 颜值:0.3, 医术:0.1, 战功:0, 文采:2.8, 政绩:0.3, 义行:0.1 }},
      { rx: /官|府|县|尹|令|相|卿|司[徒空马寇]|内史|太史|法吏|狱掾|郡守|邑[宰胥]|乡[吏绅]|史$|吏$|尉|丞|博士/, w: { 声望:1.0, 财富:0.4, 学识:0.6, 颜值:0.2, 医术:0.1, 战功:0, 文采:0.5, 政绩:2.8, 义行:0.3 }},
      { rx: /商|贾|贩|货郎|脚夫/, w: { 声望:0.6, 财富:2.8, 学识:0.2, 颜值:0.3, 医术:0.1, 战功:0, 文采:0.1, 政绩:0.4, 义行:0.1 }},
      { rx: /僧|道|观|卜|祝|祭司|巫|觋|乐师|术士/, w: { 声望:0.4, 财富:0.1, 学识:0.8, 颜值:0.2, 医术:0.4, 战功:0, 文采:0.3, 政绩:0.1, 义行:1.0 }},
      ...
      { rx: /匠|工|作|造|铸|建|织|染|缝|陶|窑|伙夫|膳夫|缀衣|车夫|舟人/, w: { 声望:0.3, 财富:0.3, 学识:0.1, 颜值:0.3, 医术:0.1, 战功:0, 文采:0, 政绩:0, 义行:0.2 }},
      // 杂役/仆从/徒隶/流民：无职业专长，至少不为「医者」
      { rx: /.+/, w: { 声望:0.1, 财富:0.1, 学识:0.1, 颜值:0.2, 医术:0.1, 战功:0, 文采:0, 政绩:0, 义行:0.2 }},
    ]
    var profile = occProfiles[7]  // 默认空职业→农夫（不要医者！）
    if (occ) {
      for (var pi = 0; pi < occProfiles.length; pi++) {
        if (occProfiles[pi].rx && occ.search(occProfiles[pi].rx) >= 0) { profile = occProfiles[pi]; break }
      }
    }
    ...
    // ── 5. 年龄分项调节（⚠️ 不能用 || 1.0，0 是有效值！）──
    // 幼儿(<6)极低乘数，少年(<12)渐长，青年(<18)接近成人
    function ageMultFor(a) {
      var m = null
      if (age < 6) m = { '声望':0.1, '财富':0.05, '学识':0.15, '颜值':1.5, '医术':0, '战功':0, '文采':0, '政绩':0, '义行':0.3 }[a]
      else if (age < 12) m = { '声望':0.25, '财富':0.15, '学识':0.35, '颜值':1.4, '医术':0.15, '战功':0.15, '文采':0.25, '政绩':0.1, '义行':0.5 }[a]
      else if (age < 18) m = { '声望':0.4, '财富':0.3, '学识':0.6, '颜值':1.3, '医术':0.3, '战功':0.3, '文采':0.5, '政绩':0.2, '义行':0.7 }[a]
      else if (age < 30) m = { '声望':0.7, '财富':0.7, '学识':0.8, '颜值':1.2, '医术':0.7, '战功':0.8, '文采':0.8, '政绩':0.6, '义行':0.9 }[a]
      else if (age <= 50) m = { '声望':1.2, '财富':1.1, '学识':1.1, '颜值':0.7, '医术':1.0, '战功':1.0, '文采':1.1, '政绩':1.3, '义行':1.0 }[a]
      else if (age <= 60) m = { '声望':1.3, '财富':1.1, '学识':1.2, '颜值':0.3, '医术':1.1, '战功':0.6, '文采':1.2, '政绩':1.3, '义行':1.1 }[a]
      else m = { '声望':1.3, '财富':0.9, '学识':1.2, '颜值':0.2, '医术':0.8, '战功':0.3, '文采':1.1, '政绩':1.2, '义行':1.2 }[a]
      return m !== undefined ? m : 1.0  // 未定义属性回退（如 颜值不在战功profile里）
    }
